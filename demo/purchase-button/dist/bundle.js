...
},{"spin.js":4}],4:[function(require,module,exports){
/**
...
var _controllersSolution = require('./controllers/solution');

var _controllersFlowClient = require('./controllers/flowClient');
...
require('domready')(function () {
	...
	var btn = new _customButton.CustomButton({
		...
		outputEventsMap: {
			'click': _controllersFlowClient.FLOW_CLIENT_EVENTS.PRODUCT_PURCHASE_REQUESTED
		}
	}, element, _controllersSolution.eventBus);

	var purchaseSolution = new _controllersSolution.PurchaseSolution({
		container: '#app'
	});
});

},{"./controllers/flowClient":10,"./controllers/solution":11,"./customButton":12,"domready":1,"front-log":2,"machina":5}],9:[function(require,module,exports){
/* global require */
...
/**
 * Т.к. FSM у Machina.js реализует интерфейс emit/on/off,
 * то в качестве шины событий будет использоваться "пустой"
 * инстанс FSM.
 */

function EventBus() {
    var bus = new Machina.Fsm({
        initialState: 'x',
        states: { 'x': {} }
    });

    bus.bindToNamespace = function (namespace) {
        return {
            emit: function emit(eventName) {
                var fullEventName = namespace + ':' + eventName;
                // console.log('Emited namespaced event :: ', fullEventName);

                for (var _len = arguments.length, rest = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
                    rest[_key - 1] = arguments[_key];
                }

                bus.emit.apply(bus, [fullEventName].concat(rest));
            }
        };
    };
...
},{"machina":5}],10:[function(require,module,exports){
/* global require */
...
var _machinaWrappers = require('../machina/wrappers');

var _controllersEventBus = require('../controllers/eventBus');
...
var STATES = {
	...
	LOADING_PRODUCTS_INFO: 'LOADING_PRODUCTS_INFO',
...
var EVENTS = {
	...
	PRODUCTS_INFO_REQUESTED: 'PRODUCTS_INFO_REQUESTED',
	PRODUCTS_LOADING_STARTED: 'PRODUCTS_LOADING_STARTED',
	PRODUCTS_LOADED: 'PRODUCTS_LOADED',
	PRODUCT_PURCHASE_REQUESTED: 'PRODUCT_PURCHASE_REQUESTED',
	PRODUCT_PURCHASING_STARTED: 'PRODUCT_PURCHASING_STARTED',
	PRODUCT_PURCHASED: 'PRODUCT_PURCHASED'
};

var FLOW_CLIENT_EVENTS = EVENTS;

exports.FLOW_CLIENT_EVENTS = FLOW_CLIENT_EVENTS;
/** for demo only */
var isZbtPurchased = false;

function FlowClient(eventBus) {
	var _STATES$READY_FOR_INTERACTION, _STATES$PURCHASING_PRODUCT, _states;

	var logPrefix = 'FlowClient FSM';

	var fsm = new Machina.Fsm({
		initialize: (0, _machinaWrappers.initialize)(logPrefix),
		initialState: STATES.INITIALIZING,

		states: (_states = {}, _defineProperty(_states, STATES.INITIALIZING, _defineProperty({
			_onEnter: function _onEnter() {
				var self = this;

				eventBus.on(EVENTS.PRODUCTS_INFO_REQUESTED, function () {
					self.handle(EVENTS.PRODUCTS_INFO_REQUESTED);
				});

				eventBus.on(EVENTS.PRODUCT_PURCHASE_REQUESTED, function (data) {
					self.handle(EVENTS.PRODUCT_PURCHASE_REQUESTED, data);
				});
			}
		}, EVENTS.PRODUCTS_INFO_REQUESTED, STATES.LOADING_PRODUCTS_INFO)), _defineProperty(_states, STATES.LOADING_PRODUCTS_INFO, _defineProperty({
			_onEnter: loadProductsInfo
		}, EVENTS.PRODUCTS_LOADED, STATES.READY_FOR_INTERACTION)), _defineProperty(_states, STATES.READY_FOR_INTERACTION, (_STATES$READY_FOR_INTERACTION = {}, _defineProperty(_STATES$READY_FOR_INTERACTION, EVENTS.SLEEP_REQUESTED, STATES.SLEEPING), _defineProperty(_STATES$READY_FOR_INTERACTION, EVENTS.PRODUCT_PURCHASE_REQUESTED, STATES.PURCHASING_PRODUCT), _STATES$READY_FOR_INTERACTION)), _defineProperty(_states, STATES.PURCHASING_PRODUCT, (_STATES$PURCHASING_PRODUCT = {
			_onEnter: purchaseProduct
		}, _defineProperty(_STATES$PURCHASING_PRODUCT, EVENTS.SLEEP_REQUESTED, STATES.SLEEPING), _defineProperty(_STATES$PURCHASING_PRODUCT, EVENTS.PRODUCT_PURCHASED, STATES.READY_FOR_INTERACTION), _STATES$PURCHASING_PRODUCT)), _states),

		run: function run() {
			this.handle(EVENTS.INITIALIZED);
			return this;
		}
	});
	...
	function loadProductsInfo() {
		eventBus.emit(EVENTS.PRODUCTS_LOADING_STARTED);

		_loadProducts().then(function (resp) {
			setTimeout(function () {
				fsm.handle(EVENTS.PRODUCTS_LOADED, resp);
				eventBus.emit(EVENTS.PRODUCTS_LOADED, resp);
			}, 1000);
		});
	}

	function purchaseProduct() {
		var productInfo = this.__lastEventArgs[0];
		eventBus.emit(EVENTS.PRODUCT_PURCHASING_STARTED);

		_loadProducts().then(function (resp) {
			if (productInfo.productId === 'test-game-zbt') isZbtPurchased = true;

			if (isZbtPurchased) resp.products[0].offer.prices = [];

			setTimeout(function () {
				fsm.handle(EVENTS.PRODUCT_PURCHASED);
				eventBus.emit(EVENTS.PRODUCT_PURCHASED, resp);
			}, 1000);
		});
	}
}
...
},{"../controllers/eventBus":9,"../machina/wrappers":13,"../tools":14,"front-log":2,"machina":5}],11:[function(require,module,exports){
/* global require */
'use strict';
...
exports.PurchaseSolution = PurchaseSolution;
...
var _flowClient = require('./flowClient');

var _widgetsPurchaseButton = require('../widgets/purchaseButton');

require('whatwg-fetch');

var rootEventBus = new _eventBus.EventBus();
exports.eventBus = rootEventBus;

function PurchaseSolution() {
	var flowClient = new _flowClient.FlowClient(rootEventBus),
	    purchaseBtnElems = document.querySelectorAll('*[data-js-widgets="purchaseButton"]');

	tools.forEach(purchaseBtnElems, function (el) {
		var _inputEventsMap;

		var btn = new _widgetsPurchaseButton.PurchaseButton({
			inputEventsMap: (_inputEventsMap = {}, _defineProperty(_inputEventsMap, _flowClient.FLOW_CLIENT_EVENTS.PRODUCTS_LOADING_STARTED, _widgetsPurchaseButton.BUTTON_EVENTS.BLOCKING_REQUESTED), _defineProperty(_inputEventsMap, _flowClient.FLOW_CLIENT_EVENTS.PRODUCTS_LOADED, _widgetsPurchaseButton.BUTTON_EVENTS.PRODUCTS_LOADED), _defineProperty(_inputEventsMap, _flowClient.FLOW_CLIENT_EVENTS.PRODUCT_PURCHASING_STARTED, _widgetsPurchaseButton.BUTTON_EVENTS.BLOCKING_REQUESTED), _defineProperty(_inputEventsMap, _flowClient.FLOW_CLIENT_EVENTS.PRODUCT_PURCHASED, _widgetsPurchaseButton.BUTTON_EVENTS.PRODUCT_PURCHASED), _inputEventsMap),
			outputEventsMap: {
				'click': _flowClient.FLOW_CLIENT_EVENTS.PRODUCT_PURCHASE_REQUESTED
			}
		}, el, rootEventBus);
	});

	rootEventBus.emit(_flowClient.FLOW_CLIENT_EVENTS.PRODUCTS_INFO_REQUESTED);
}

},{"../tools":14,"../widgets/purchaseButton":15,"./eventBus":9,"./flowClient":10,"whatwg-fetch":7}],12:[function(require,module,exports){
/* global require */
...
var STATES = {
	...
	PRODUCTS_UPDATING: 'PRODUCTS_UPDATING',
	BLOCKED: 'BLOCKED',
	DISABLED: 'DISABLED',
	NEXT_STATE_RESOLVING: 'NEXT_STATE_RESOLVING',
	PURCHASE_RESULT_PROCESSING: 'PURCHASE_RESULT_PROCESSING'
};

var EVENTS = {
	INITIALIZED: 'INITIALIZED',
	CLICK_REQUESTED: 'CLICK_REQUESTED',
	BLOCKING_REQUESTED: 'BLOCKING_REQUESTED',
	DISABLE_REQUESTED: 'DISABLE_REQUESTED',
	READY_REQUESTED: 'READY_REQUESTED',
	PRODUCTS_LOADED: 'PRODUCTS_LOADED',
	PRODUCTS_UPDATED: 'PRODUCTS_UPDATED',
	PRODUCT_PURCHASED: 'PRODUCT_PURCHASED',
	RESOLVE_REQUESTED: 'RESOLVE_REQUESTED'
};

var BUTTON_EVENTS = EVENTS;

exports.BUTTON_EVENTS = BUTTON_EVENTS;

function CustomButton(config, element, eventBus) {
	var _STATES$READY_FOR_INTERACTION, _STATES$BLOCKED, _STATES$DISABLED, _STATES$NEXT_STATE_RESOLVING, _states;

	var products = null;
	var spinner = Ladda.create(element);

	var fsm = new Machina.Fsm({
		initialize: (0, _machinaWrappers.initialize)('FSM ' + element.id),
		initialState: STATES.INITIALIZING,

		states: (_states = {}, _defineProperty(_states, STATES.INITIALIZING, _defineProperty({}, EVENTS.INITIALIZED, function () {
			var self = this;

			// Map DOM events to FSM events
			element.addEventListener('click', function () {
				self.handle(EVENTS.CLICK_REQUESTED);
			});

			// Subscribes to all possible input events

			var _loop = function (inputEvent) {
				eventBus.on(inputEvent, function (data) {
					self.handle(config.inputEventsMap[inputEvent], data);
				});
			};

			for (var inputEvent in config.inputEventsMap) {
				_loop(inputEvent);
			}

			self.transition(STATES.READY_FOR_INTERACTION);
		})), _defineProperty(_states, STATES.READY_FOR_INTERACTION, (_STATES$READY_FOR_INTERACTION = {}, _defineProperty(_STATES$READY_FOR_INTERACTION, EVENTS.BLOCKING_REQUESTED, STATES.BLOCKED), _defineProperty(_STATES$READY_FOR_INTERACTION, EVENTS.DISABLING_REQUESTED, STATES.DISABLED), _defineProperty(_STATES$READY_FOR_INTERACTION, EVENTS.CLICK_REQUESTED, STATES.CLICK_PROCESSING), _STATES$READY_FOR_INTERACTION)), _defineProperty(_states, STATES.CLICK_PROCESSING, _defineProperty({
			_onEnter: processClick
		}, EVENTS.BLOCKING_REQUESTED, STATES.BLOCKED)), _defineProperty(_states, STATES.PRODUCTS_UPDATING, _defineProperty({
			_onEnter: updateProducts
		}, EVENTS.RESOLVE_REQUESTED, STATES.NEXT_STATE_RESOLVING)), _defineProperty(_states, STATES.PURCHASE_RESULT_PROCESSING, _defineProperty({
			_onEnter: processPurchaseResult
		}, EVENTS.RESOLVE_REQUESTED, STATES.NEXT_STATE_RESOLVING)), _defineProperty(_states, STATES.BLOCKED, (_STATES$BLOCKED = {
			_onEnter: blockButton
		}, _defineProperty(_STATES$BLOCKED, EVENTS.PRODUCTS_LOADED, STATES.PRODUCTS_UPDATING), _defineProperty(_STATES$BLOCKED, EVENTS.PRODUCT_PURCHASED, STATES.PURCHASE_RESULT_PROCESSING), _STATES$BLOCKED)), _defineProperty(_states, STATES.DISABLED, (_STATES$DISABLED = {
			_onEnter: disableButton
		}, _defineProperty(_STATES$DISABLED, EVENTS.PRODUCTS_LOADED, STATES.PRODUCTS_UPDATING), _defineProperty(_STATES$DISABLED, EVENTS.PRODUCT_PURCHASED, STATES.PURCHASE_RESULT_PROCESSING), _STATES$DISABLED)), _defineProperty(_states, STATES.NEXT_STATE_RESOLVING, (_STATES$NEXT_STATE_RESOLVING = {
			_onEnter: resolveNextState

		}, _defineProperty(_STATES$NEXT_STATE_RESOLVING, EVENTS.READY_REQUESTED, STATES.READY_FOR_INTERACTION), _defineProperty(_STATES$NEXT_STATE_RESOLVING, EVENTS.DISABLE_REQUESTED, STATES.DISABLED), _defineProperty(_STATES$NEXT_STATE_RESOLVING, '_onExit', function _onExit() {
			unblockButton.call(this);
			enableButton.call(this);
		}), _STATES$NEXT_STATE_RESOLVING)), _states),
	...
	function processClick() {
		this.handle(EVENTS.BLOCKING_REQUESTED);

		eventBus.emit(config.outputEventsMap['click'], {
	...
	function updateProducts() {
		products = (this.__lastEventArgs[0] || {}).products || [];
		this.handle(EVENTS.RESOLVE_REQUESTED);
	}

	function processPurchaseResult() {
		products = (this.__lastEventArgs[0] || {}).products || [];
	...
	function unblockButton() {
		spinner.stop();
		this.handle(EVENTS.READY_REQUESTED);
	}
	...
	function resolveNextState() {
		if (null === products) throw new Error('Incorect state transition');

		var productId = element.getAttribute('data-product-id'),
		    filtered = products.filter(function (x) {
			return x.productId === productId;
		}),
		    productInfo = filtered.length ? filtered[0] : null,
		    canPurchase = productInfo && !!productInfo.offer.prices.length;

		// console.log('resolveNextState :: ', this);

		if (canPurchase) this.handle(EVENTS.READY_REQUESTED);else this.handle(EVENTS.DISABLE_REQUESTED);
	}
}

},{"./controllers/eventBus":9,"./machina/wrappers":13,"./tools":14,"front-log":2,"ladda":3,"machina":5}],13:[function(require,module,exports){
/* global require */
...
/**
 * Враппер над FSM.initialize, добавляющий логирование
 *
 * returns {Function}
 */

function initialize() {
    var prefix = arguments.length <= 0 || arguments[0] === undefined ? 'FSM' : arguments[0];

    return function (options) {
        var oldTrans = this.transition;
        var oldHandle = this.handle;
        var prevTransMsg, prevHandleMsg;
        ...
        this.transition = function () {
            ...
            // грязный хак, т.к. функция вариадическая
            // и аргумент, содержащий id состояния, приходит не всегда
            // в одинаковой позиции.
            var state = args.length > 1 && args[1] ? args[1] : args[0];

            var newMsg = prefix + ' :: =====>> :: "' + state + '" state';
            if (prevTransMsg !== newMsg) {
                log.debug(newMsg);
                prevTransMsg = newMsg;
            }
        ...
        /**
         * Враппер над методом FSM `.handle`, используемым для
         * генерации событий в рамках инстанса FSM.
         * Добавляет логирование и сохранение аргументов, с которым
         * было сгенерированно событие.
         * Аргументы запоминаются в спец. поле `__lastEventArgs`.
         * Этот костыль нужен потому,  Machina.js  не прокидывает
         * аргументы, переданные в `handle()` до метода `_onEnter()`.
         *
         * returns {Function}
         */
        this.handle = function (eventName) {
            if (typeof eventName !== 'string') return this;
            var newMsg = prefix + ' :: <<===== :: "' + eventName + '" event in state "' + this.state + '"';
            if (prevHandleMsg !== newMsg) {
                log.debug(newMsg);
                prevHandleMsg = newMsg;
            }
            ...
            this.__lastEventArgs = rest;
            return oldHandle.apply(this, [eventName].concat(rest));
        };
    };
}

},{"front-log":2}],14:[function(require,module,exports){
"use strict";
...
function forEach(xs, fn) {
	return Array.prototype.forEach.call(xs, fn);
}
...
function PurchaseButton(config, element, eventBus) {
	var _STATES$READY_FOR_INTERACTION, _STATES$BLOCKED, _STATES$DISABLED, _STATES$NEXT_STATE_RESOLVING, _states;
...
},{"../controllers/eventBus":9,"../machina/wrappers":13,"../tools":14,"front-log":2,"ladda":3,"machina":5}]},{},[8])
